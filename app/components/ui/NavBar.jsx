...
const NavBar = () => {
    ...
    return (
    <Navbar dark className="absolute" onMenuOpenChange={setToggleMenu}>
        <NavbarBrand>
            <Image 
        ...
        <NavbarContent justify="center" className="hidden md:flex gap-8">
            {navItems.map((item) => (
                <NavbarItem><Link className={`transition border-b-2 border-transparent flex h-full text-xl text-center hover:border-white ${pathname === item.href.toLowerCase() ? 'border-b-secondary font-bold hover:border-b-secondary' : ''}`} href={item.href}>{item.label}</Link></NavbarItem>
            ))}
        </NavbarContent>
        <NavbarMenuToggle
